...
class Node: 
  ...
  def size_less_than(self, target_size):
    total = 0
    ...
    for k,v in self.childDirs.items():
      total += v.size_less_than(target_size)
  ...
  def all_dir_sizes(self):
    ...
    for k,v in self.childDirs.items():
      all_sizes.extend(v.all_dir_sizes())
...
def run():
  ...
  space_needed = TARGET_UNUSED_SPACE - unused_space
  # print(f"{space_needed:,}")
  
  all_size = root.all_dir_sizes()
  
  print(min([s for s in all_size if s >= space_needed]))
